...
- Parâmetros:
  - `$separator`: Caractere ou texto usado como separador.
...
#### setCalendars(int $calendars)
Define o número de calendários a serem exibidos.

- Parâmetros:
  - `$calendars`: Número de calendários.
...
- Parâmetros:
  - `$mask`: Máscara para o banco de dados.
...
#### setSteps($stepHours, $stepMinutes, $stepSeconds)
Define os incrementos para horas, minutos e segundos nos seletores de tempo.

- Parâmetros:
  - `$stepHours`: Incremento das horas.
  - `$stepMinutes`: Incremento dos minutos.
  - `$stepSeconds`: Incremento dos segundos.
...
- Parâmetros:
  - `$grid`: Número que define a grade.
