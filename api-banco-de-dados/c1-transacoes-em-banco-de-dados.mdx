...
Um exemplo de arquivo de conexão com o BD seria:

    host   = localhost
    port   = 
    name   = erp
    user   = erp_consulta
    pass   = erp_consulta#1337
    type   = pgsql

## 1.1. Aconteceu um problema, como abortar a transação?
...
## 1.3. Como posso executar um comando SQL manual?
...
## 1.4. Como abrir conexão manual com um banco de dados
...
## 1.5. Registros de log da transação
